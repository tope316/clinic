...
class Bottles_Model extends CI_Model {
	
	function get_data()
	{
		$sqlCount = "select count(*) as xcount from bottles";
		...
		$sqlInfo = "select ID, description from bottles WHERE 1=1";
	...
	function del()
	{
		...
		$qry = "delete from bottles where ID = ?";
	...
	function get_record()
	{
		...
		$qry = $this->db->query("Select ID, description FROM bottles where ID = $code");
	...
	{						
		...
		$this->db->where('ID',$id);
		$res = $this->db->update('bottles', $data); 
	...
	public function add_save()
	{
		...
		$res = $this->db->insert('bottles', $data); 
	...
	function get_list()
	{			
		$qry = $this->db->query("select * from bottles WHERE 1=1");
