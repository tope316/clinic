...
class Durations_Model extends CI_Model {
	
	function get_data()
	{
		$sqlCount = "select count(*) as xcount from durations";
		...
		$sqlInfo = "select ID, description from durations WHERE 1=1";
	...
	function del()
	{
		...
		$qry = "delete from durations where ID = ?";
	...
	function get_record()
	{
		...
		$qry = $this->db->query("Select ID, description FROM durations where ID = $code");
	...
	{						
		...
		$this->db->where('ID',$id);
		$res = $this->db->update('durations', $data); 
	...
	public function add_save()
	{
		...
		$res = $this->db->insert('durations', $data); 
	...
	function get_list()
	{			
		$qry = $this->db->query("select * from durations WHERE 1=1");
