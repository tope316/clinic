...
class Generic_Model extends CI_Model {
	...
	function get_data()
	{
		$sqlCount = "select count(*) as xcount from generic";
		...
		$sqlInfo = "select ID, description from generic WHERE 1=1";
	...
	function del()
	{
		...
		$qry = "delete from generic where ID = ?";
	...
	function get_record()
	{
		...
		$qry = $this->db->query("Select ID, description FROM generic where ID = $code");
	...
	{						
		...
		$this->db->where('ID',$id);
		$res = $this->db->update('generic', $data); 
	...
	public function add_save()
	{
		...
		$res = $this->db->insert('generic', $data); 
	...
	function get_list()
	{			
		$qry = $this->db->query("select * from generic WHERE 1=1");
