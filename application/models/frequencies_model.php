...
class Frequencies_Model extends CI_Model {
	
	function get_data()
	{
		$sqlCount = "select count(*) as xcount from frequencies";
		...
		$sqlInfo = "select ID, description from frequencies WHERE 1=1";
	...
	function del()
	{
		...
		$qry = "delete from frequencies where ID = ?";
	...
	function get_record()
	{
		...
		$qry = $this->db->query("Select ID, description FROM frequencies where ID = $code");
	...
	{						
		...
		$this->db->where('ID',$id);
		$res = $this->db->update('frequencies', $data); 
	...
	public function add_save()
	{
		...
		$res = $this->db->insert('frequencies', $data); 
	...
	function get_list()
	{			
		$qry = $this->db->query("select * from frequencies WHERE 1=1");
