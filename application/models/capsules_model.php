...
class Capsules_Model extends CI_Model {
	
	function get_data()
	{
		$sqlCount = "select count(*) as xcount from capsules";
		...
		$sqlInfo = "select ID, description from capsules WHERE 1=1";
	...
	function del()
	{
		...
		$qry = "delete from capsules where ID = ?";
	...
	function get_record()
	{
		...
		$qry = $this->db->query("Select ID, description FROM capsules where ID = $code");
	...
	{						
		...
		$this->db->where('ID',$id);
		$res = $this->db->update('capsules', $data); 
	...
	public function add_save()
	{
		...
		$res = $this->db->insert('capsules', $data); 
	...
	function get_list()
	{			
		$qry = $this->db->query("select * from capsules WHERE 1=1");
